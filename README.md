# mathematics

An educational web project to learn the things you needed

🏫 1. SCHOOL & ACADEMIC KNOWLEDGE
🧮 Mathematics

You should gradually learn:

Counting and place value
Addition, subtraction, multiplication, division
Fractions
Decimals
Percentages
Ratios
Geometry
Measurement
Time
Money
Data and graphs
Probability
Algebra
Equations
Statistics
Problem-solving
Later: advanced mathematics and calculus

Don't just memorize formulas.

Learn → understand → practise → solve problems → explain it to someone.

For example, if you're learning fractions, don't only learn 1/2 + 1/4. Understand what the fractions actually represent.

